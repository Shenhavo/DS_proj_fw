...
//    HAL_NVIC_DisableIRQ(DCMI_VSYNC_IRQN); // TODO: find out what is this


    // Clear fb_enabled flag
    // This is executed only once to initialize the FB enabled flag.
//    JPEG_FB()->enabled = 0; // TODO: uncomment

    // Set default color palette.
//    sensor.color_palette = rainbow_table; // TODO: uncomment
...
int sensor_set_hmirror(int enable)
{
    ...
    HAL_Delay(100); // systick_sleep(100); // wait for the camera to settle
    return 0;
}
...
int sensor_set_vsync_output(GPIO_TypeDef *gpio, uint32_t pin)
{
    ...
    NVIC_SetPriority(DCMI_VSYNC_IRQN, 0); // TODO: NVIC_SetPriority(DCMI_VSYNC_IRQN, IRQ_PRI_EXTINT);
